...
默认的严重级别的错误，如果要其它级别则会单独说明。

## 待办任务
每完成一项 todo，你需要用 - [x] 标记完成。实现诊断规则要有单元测试。
...
- [x] 实现诊断规则 BCX31396：
    - [x] 受限类型变量不能为 Nullable, 例如 `Dim x As Span(Of T)?` 和 `Dim x? As Span(Of T)` 都是错误的
    - [x] 受限类型不能是数组类型, 例如 `Dim x As Span(Of T)()` 和 `Dim x() As Span(Of T)` 都是错误的
    - [x] 受限类型不能是集合初始化表达式成员，例如你不能把 Span 放在 数组/List/Dictionary 的初始化表达式中
    - [x] 如果一个结构体不是受限类型，那它的的字段或属性都不能是受限类型
    - [x] 类的字段或属性类型不能是受限类型
    - [x] 匿名类型的成员不能是受限类型
    - [x] ByRef 参数类型不能是受限类型
    - [x] 返回值不能是受限类型
