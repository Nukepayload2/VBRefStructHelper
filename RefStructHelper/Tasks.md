...
- [ ] 实现诊断规则 BCX31396：
    - [ ] 受限类型变量不能为 Nullable, 例如 `Dim x As Span(Of T)?` 和 `Dim x? As Span(Of T)` 都是错误的
    - [ ] 受限类型不能是数组类型, 例如 `Dim x As Span(Of T)()` 和 `Dim x() As Span(Of T)` 都是错误的
    - [ ] 受限类型不能是集合初始化表达式成员，例如你不能把 Span 放在 数组/List/Dictionary 的初始化表达式中
    - [ ] 如果一个结构体不是受限类型，那它的的字段或属性都不能是受限类型
    - [ ] 类的字段或属性类型不能是受限类型
    - [ ] 匿名类型的成员不能是受限类型
    - [ ] ByRef 参数类型不能是受限类型
    - [ ] 返回值不能是受限类型（这个是警告）
